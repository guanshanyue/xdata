...
def mysql_valid(ip,user,password,port,db):
    try:
        ip = ip
        user = user
        password = password
        ...
        try:
            with MysqlClient(ip=ip, user=user, password=password, port=port,db=db) as f:
                res = f.tablename()
                #res = f.gen_alter('user_hosts')
                for i in res:
...
if __name__ == "__main__":
    print(mysql_valid('127.0.0.1','root','111111',3306,'doms'))
    #db = MysqlClient(ip='127.0.0.1',user='root',password='111111',port=3306,db='doms')
    #print(db)
    #print(db.index('user_hosts'))
