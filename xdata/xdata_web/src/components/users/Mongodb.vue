    ...
    <div>
        <el-row>
            ...
            <el-col :span="8"  style="text-align: right">
                ...
                <el-button v-if="has_permission('users_mongodb_add')" type="primary" @click="handleAdd">添加数据库</el-button>
            </el-col>
        </el-row>
        <el-table :data="hosts.data" @expand-change="get_mongodb_extend" v-loading="tableLoading" style="width: 100%; margin-top: 20px">
            ...
            <el-table-column label="操作" width="450px" v-if="has_permission('users_mongodb_edit|users_mongodb_del|users_mongodb_valid')">
                <template slot-scope="scope">
                    <el-button v-if="has_permission('users_mongodb_edit')" size="small" @click="handleEdit(scope.row)">编辑</el-button>
                    ...
                    <el-button v-if="has_permission('users_mongodb_valid')" size="small" type="success" @click="valid(scope.row)"
                               :loading="btnValidLoading[scope.row.id]">验证
                    </el-button>
                    <el-button v-if="has_permission('users_mongodb_del')" size="small" type="danger" @click="deleteCommit(scope.row)"
            ...
            <el-tabs v-model="activeName" >
                <el-tab-pane label="单条记录" name="first">
                    <el-form :model="form" label-width="80px">
                        ...
                        <el-form-item  label="认证数据库" prop="db_database" required>
                            <el-input type="db_database" v-model="form.db_database" value="admin"></el-input>
                        </el-form-item>
                ...
                <el-tab-pane label="批量导入" name="second" v-if="importStatus" v-loading="import_loading" element-loading-text="正在导入...">
                    ...
                    <el-upload ref="upload" action="" :http-request="import_submit" name="file" :multiple="false"
                        ...
                        <el-button size="small" type="primary" v-if="has_permission('users_mongodb_add')">点击批量导入</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //获取区域
            get_mongodb_zone () {
                this.$http.get('/api/users/mongodb/zone/').then(res => {
            ...
            //获取数据库类型
            get_mongodb_type () {
                this.$http.get('/api/users/mongodb/type/').then(res => {
            ...
            get_mongodb_extend (row, expanded) {
                if (expanded) {
                    this.$http.get(`/api/users/mongodb/${row.id}/extend/`).then(res => {
            ...
            import_submit(data){
                ...
                this.$http.post(`/api/users/mongodb/import`, excel_file).then(res => {
            ...
            saveCommit () {
                ...
                let request;
                if (this.form.id) {
                    request = this.$http.put(`/api/users/mongodb/${this.form.id}`, this.form)
                } else {
                    request = this.$http.post(`/api/users/mongodb/`, this.form)
                }
                request.then(() => {
                    this.dialogVisible = false;
                    this.$layer_message('提交成功', 'success');
                    this.fetch(this.currentPage);
                    this.get_mongodb_zone();
                    this.get_mongodb_type()
            ...
            deleteCommit (row) {
                this.$confirm('此操作将永久删除该主机，是否继续？', '删除确认', {type: 'warning'}).then(() => {
                    ...
                    this.$http.delete(`/api/users/mongodb/${row.id}`).then(() => {
                        this.fetch(this.currentPage)
            ...
            valid (row) {
                ...
                this.$http.get(`/api/users/mongodb/${row.id}/valid`).then(() => {
